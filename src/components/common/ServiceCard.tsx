...
const ServiceCard: React.FC<ServiceCardProps> = ({ icon, title, description, category }) => {
  ...
  // Add event listener for the document to continue tracking mouse outside the card
  useEffect(() => {
  ...
  return (
    <div 
      ref={cardRef}
      className="card-spotlight h-full flex flex-col" 
      onMouseMove={handleMouseMove}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      <div className="service-icon">{icon}</div>
      <div className="service-badge">{category}</div>
